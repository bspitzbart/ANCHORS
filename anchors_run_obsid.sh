...
if [ $nsol -gt 1 ]
  then
    echo "Need to merge asol files"
    /home/anchors/bin/merge_asol.sh $obsid $obsdir $logfile
fi
...
if [ $badprocess -eq 1 ] 
	...
	echo "ANCHORS: Reprocessing level 1 file with reprocess level1" >> $logfile 
	/home/anchors/bin/reprocess_level1.sh $obsid $obsdir $logfile
        evt2file="acis_evt2.fits"
fi
#----------------------------------------
#[SOURCE DETECTION]
#----------------------------------------
echo "ANCHORS:Starting source detection" >> $logfile
dmcopy infile=$evt2file'[energy=300:8000]' outfile=evt2_efilt.fits kernel=DEFAULT clobber+ verbose=2 1>> $logfile 2>&1

ln -s $obsdir/evt2_efilt.fits $wavdir/$obsid2/0.3-8.0/evt2_efilt.fits
cd $wavdir/$obsid2/0.3-8.0/
ls $obsdir/*asol* > asol1.lis
/home/anchors/bin/wave_run.csh $obsid2 $logfile
...
dmlist 'evt2_efilt_src.fits[cols shape,x,y,r,rotang]' data outfile=src.reg  1>> $logfile 2>&1
...
idl <<EOF
.run $yaxx/bin/mk_psf_ellipse_exc.pro
...
unset UPARM
# set path for new yaxx (perl)
PATH=/home/anchors/bin:$PATH
export PATH
$yaxx/bin/yaxx/yaxx 1>> $logfile 2>&1
